 ...
 */
{
	...
	const TiltObj = function(el, options) {
		...
		this.DOM = {};
		this.DOM.img = this.el.querySelector('.bcontent__img');
		this.DOM.title = this.el.querySelector('.bcontent__title');
		this._initEvents();
	}
	...
	DOM.contentElems = Array.from(document.querySelectorAll('.bcontent-wrap'));
	DOM.contentLinks = Array.from(document.querySelectorAll('.bcontent'));
	DOM.footer = document.querySelector('.bcontent--related');
	...
	const init = function() {
		imagesLoaded(document.body, () => {
			...
			Array.from(document.querySelectorAll('.bcontent--layout')).forEach(el => new TiltObj(el));
	...
	document.getElementById("mail").addEventListener("click",function(e){
		...
		document.location="mailto:himichhabra14@gmail.com?subject=I%20wanna%20hire%20you";
	});
};
